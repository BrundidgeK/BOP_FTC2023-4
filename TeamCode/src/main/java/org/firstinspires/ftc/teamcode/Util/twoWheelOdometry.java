...
public class twoWheelOdometry {
    ...
    public static final double INCHES_PER_REVOLUTION = ((48.0/25.4 * Math.PI) / 2000); // Odom wheel Circumference(inches) / (Ticks Per Revolution of odom pods, google it)
    ...
    public twoWheelOdometry(HardwareMap hw){
        ...
        hori = hw.get(DcMotor.class, "suspend");
        vert = hw.get(DcMotor.class, "Intake");
    ...
    public double[] moveTowards(Vector2D diff) {
    ...
    public Vector2D difference(Vector2D target){
        return new Vector2D(target.x - x, target.y - y);
    }
